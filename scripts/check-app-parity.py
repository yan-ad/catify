...
manifest=json.loads(a.manifest.read_text())['commands']; missing=sorted(inv-set(manifest)); extra=sorted(set(manifest)-inv); invalid=[n for n,v in manifest.items() if v.get('status') not in {'native','adapter','blocked-live'} or not isinstance(v.get('owner'),int) or not isinstance(v.get('json'),bool) or not isinstance(v.get('non_interactive'),bool)]
