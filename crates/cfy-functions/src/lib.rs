...
pub const FUNCTIONS_API_BASE: &str =
    "https://app.shopify.com/services/partners/api/functions/unstable";
...
pub enum FunctionsError {
    ...
    #[error(
        "no Shopify Function extensions were found; add a Function extension or pass its directory"
    )]
    NoFunctions,
    ...
    #[error(
        "function WebAssembly output is missing at {wasm}; configure build.command or build the function externally"
    )]
...
fn configuration_table(raw: &Table) -> Result<&Table> {
    if let Some(extensions) = raw.get("extensions").and_then(Value::as_array) {
        return extensions
            ...
            .ok_or_else(|| {
                FunctionsError::InvalidConfig("[[extensions]] contains no Function entry".into())
            });
    }
    Ok(raw)
}
...
pub fn function_specs(graph: &AppConfigGraph) -> Result<Vec<FunctionSpec>> {
    graph
        ...
        .filter(|extension| {
            extension.family == ExtensionFamily::Function || modern_function(extension)
        })
        .map(FunctionSpec::from_extension)
        .collect()
}
...
fn modern_function(extension: &ExtensionConfig) -> bool {
    extension
        ...
        .flatten()
        .filter_map(Value::as_table)
        .any(|table| string(table, "type").as_deref().map(is_function_type) == Some(true))
}
...
pub fn select_function(graph: &AppConfigGraph, directory: Option<&Path>) -> Result<FunctionSpec> {
    ...
    if functions.is_empty() {
        return Err(FunctionsError::NoFunctions);
    }
    if let Some(directory) = directory {
        if let Some(found) = functions
            .iter()
            .find(|function| function.directory == directory)
        {
            return Ok(found.clone());
        }
        return Err(FunctionsError::FunctionNotFound {
            directory: directory.to_path_buf(),
            choices: choices(&functions),
        });
    }
    if functions.len() == 1 {
        return Ok(functions[0].clone());
    }
    Err(FunctionsError::FunctionSelectionRequired(choices(
        &functions,
    )))
}
        ...
        targeting: spec
            ...
            .map(|t| {
                (
                    t.target.clone(),
                    TargetInfo {
                        input_query: t.input_query.clone(),
                        export: t.export.clone(),
                    },
                )
            })
            .collect(),
        schema_path: spec.schema_path.exists().then(|| spec.schema_path.clone()),
        wasm_path: spec.wasm_path.clone(),
        function_runner_path: runner_path,
    }
}
...
pub async fn build(spec: &FunctionSpec, supervisor: &Supervisor) -> Result<Option<ProcessOutput>> {
    let Some(command) = spec.build.command.as_deref() else {
        return if spec.wasm_path.is_file() {
            Ok(None)
        } else {
            Err(FunctionsError::MissingBuild {
                wasm: spec.wasm_path.clone(),
            })
        };
    };
    ...
    if !spec.wasm_path.is_file() {
        return Err(FunctionsError::MissingBuildOutput(spec.wasm_path.clone()));
    }
    Ok(Some(output))
}
...
pub fn typegen_process_spec(spec: &FunctionSpec) -> ProcessSpec {
    if let Some(command) = spec.build.typegen_command.as_deref() {
        let (program, args) = shell_command(command);
        return ProcessSpec::new(program)
            .args(args)
            .current_dir(&spec.directory)
            .output(OutputMode::Inherit);
    }
...
fn ensure_success(program: &str, status: &ExitStatus) -> Result<()> {
    ...
    Err(FunctionsError::ProcessFailed {
...
pub fn runner_download_url(platform: RunnerPlatform) -> String {
    let target = match platform {
        RunnerPlatform::MacArm => "arm-macos",
        RunnerPlatform::MacX86_64 => "x86_64-macos",
        RunnerPlatform::LinuxArm => "arm-linux",
        RunnerPlatform::LinuxX86_64 => "x86_64-linux",
        RunnerPlatform::WindowsX86_64 => "x86_64-windows",
    };
    format!(
        "https://github.com/Shopify/function-runner/releases/download/v{FUNCTION_RUNNER_VERSION}/function-runner-{target}-v{FUNCTION_RUNNER_VERSION}.gz"
    )
}

pub fn current_runner_platform() -> Result<RunnerPlatform> {
    runner_platform(env::consts::OS, env::consts::ARCH)
}
pub fn runner_platform(os: &str, arch: &str) -> Result<RunnerPlatform> {
    match (os, arch) {
        ("macos", "aarch64") => Ok(RunnerPlatform::MacArm),
        ("macos", "x86_64") => Ok(RunnerPlatform::MacX86_64),
        ("linux", "aarch64") => Ok(RunnerPlatform::LinuxArm),
        ("linux", "x86_64") => Ok(RunnerPlatform::LinuxX86_64),
        ("windows", "x86_64") => Ok(RunnerPlatform::WindowsX86_64),
        _ => Err(FunctionsError::UnsupportedPlatform {
            os: os.into(),
            arch: arch.into(),
        }),
    }
...
pub fn cached_runner_path(cache_root: &Path) -> PathBuf {
    let name = if cfg!(windows) {
        format!("function-runner-{FUNCTION_RUNNER_VERSION}.exe")
    } else {
        format!("function-runner-{FUNCTION_RUNNER_VERSION}")
    };
    cache_root.join("functions").join(name)
}

pub fn runner_override_from_env() -> Result<Option<PathBuf>> {
    let Some(value) = env::var_os("CFY_FUNCTION_RUNNER_BIN") else {
        return Ok(None);
    };
    let path = PathBuf::from(value);
    if !path.is_file() {
        return Err(FunctionsError::InvalidRunnerOverride(path));
    }
    Ok(Some(path))
}

pub fn resolve_runner(cache_root: &Path) -> Result<PathBuf> {
    resolve_runner_with_override(
        cache_root,
        env::var_os("CFY_FUNCTION_RUNNER_BIN").as_deref(),
    )
}

pub fn resolve_runner_with_override(
    cache_root: &Path,
    ...
    download_runner(
        &destination,
        &runner_download_url(current_runner_platform()?),
    )?;
    Ok(destination)
}

pub fn download_runner(destination: &Path, url: &str) -> Result<()> {
    let parsed = Url::parse(url).map_err(|e| FunctionsError::RunnerDownload {
        url: url.into(),
        message: e.to_string(),
    })?;
    if !matches!(parsed.scheme(), "https" | "http")
        || parsed
            ...
            .any(|p| p == "..")
    {
        return Err(FunctionsError::RunnerDownload {
            url: url.into(),
            message: "unsafe download URL".into(),
        });
    }
    install_crypto_provider();
    let response = reqwest::blocking::Client::builder()
        .build()
        .map_err(|e| FunctionsError::RunnerDownload {
            url: url.into(),
            message: e.to_string(),
        })?
        .get(parsed)
    ...
    let parent = destination
        .parent()
        .ok_or_else(|| FunctionsError::InvalidConfig("runner cache path has no parent".into()))?;
    ...
    let result = (|| -> Result<()> {
        ...
        let mut output = File::create(&temp)
            .map_err(|source| io_error("creating runner temporary file", source))?;
        io::copy(&mut decoder, &mut output)
            .map_err(|source| io_error("decompressing function-runner", source))?;
        output
            .sync_all()
            .map_err(|source| io_error("syncing function-runner", source))?;
        #[cfg(unix)]
        {
            use std::os::unix::fs::PermissionsExt;
            fs::set_permissions(&temp, fs::Permissions::from_mode(0o755))
                .map_err(|source| io_error("marking function-runner executable", source))?;
        }
        atomic_replace(&temp, destination)
            .map_err(|source| io_error("installing function-runner", source))
    })();
    if result.is_err() {
...
fn sibling_temp_path(path: &Path) -> PathBuf {
    let mut name = path
        .file_name()
        .unwrap_or_else(|| OsStr::new("function-runner"))
        .to_os_string();
...
fn io_error(context: impl Into<String>, source: io::Error) -> FunctionsError {
    FunctionsError::Io {
...
pub fn run_process_spec(
    spec: &FunctionSpec,
    runner: &Path,
    options: &RunOptions,
) -> Result<ProcessSpec> {
    if options.schema_path.is_some() != options.query_path.is_some() {
        return Err(FunctionsError::InvalidConfig(
            "schema_path and query_path must be supplied together".into(),
        ));
    }
    ...
    if let Some(export) = &options.export {
        args.extend(["--export".into(), export.clone()]);
    }
    if options.json {
        args.push("--json".into());
    }
    ...
    Ok(ProcessSpec::new(runner.to_string_lossy().into_owned())
        .args(args)
        .current_dir(&spec.directory)
        .output(OutputMode::Inherit))
}
...
pub async fn run(
    ...
    let output = supervisor
        .spawn(run_process_spec(spec, runner, options)?)?
        .wait_with_signal_forwarding()
        .await?;
    ensure_success(&runner.to_string_lossy(), &output.status)?;
    Ok(output)
}
...
pub struct LogFileMetadata {
    ...
    pub function_handle: String,
    pub identifier: String,
}

pub fn parse_log_filename(filename: &str) -> Option<LogFileMetadata> {
    let fields: Vec<_> = filename.split(&['_', '.'][..]).collect();
    (fields.len() >= 6).then(|| LogFileMetadata {
        namespace: fields[3].into(),
        function_handle: fields[4].into(),
        identifier: fields[5].into(),
    })
}
...
pub fn list_replay_runs(log_directory: &Path, function_handle: &str) -> Result<Vec<ReplayRun>> {
    ...
    let mut names: Vec<_> = fs::read_dir(log_directory)
        .map_err(|source| io_error("reading function logs", source))?
    ...
    for name in names {
        if runs.len() >= LOG_SELECTOR_LIMIT {
            break;
        }
        let name = name.to_string_lossy();
        let Some(meta) = parse_log_filename(&name) else {
            continue;
        };
        if meta.namespace != "extensions" || meta.function_handle != function_handle {
            continue;
        }
        let path = log_directory.join(name.as_ref());
        let text =
            fs::read_to_string(&path).map_err(|source| io_error("reading function log", source))?;
        let mut data: FunctionRunData =
            serde_json::from_str(&text).map_err(|e| FunctionsError::InvalidLog {
                path: path.clone(),
                message: e.to_string(),
            })?;
        if data.payload.input.is_none() {
            continue;
        }
...
pub fn select_replay_run(
    ...
    if let Some(identifier) = identifier {
        return runs
            .into_iter()
            .find(|run| run.data.identifier == identifier)
            .ok_or_else(|| FunctionsError::LogNotFound {
                identifier: identifier.into(),
                directory: log_directory.into(),
                function_handle: function_handle.into(),
            });
    }
    match runs.len() {
        0 => Err(FunctionsError::NoReplayLogs(log_directory.into())),
        1 => Ok(runs.into_iter().next().unwrap()),
        _ => Err(FunctionsError::ReplaySelectionRequired(
            runs.iter()
                .map(|r| ReplayChoice {
                    identifier: r.data.identifier.clone(),
                    path: r.path.clone(),
                })
                .collect(),
        )),
...
pub fn replay_input(run: &ReplayRun) -> Result<(Vec<u8>, Option<String>)> {
    let input = run
        ...
        .input
        .as_ref()
        .ok_or_else(|| FunctionsError::InvalidLog {
            path: run.path.clone(),
            message: "payload.input is null".into(),
        })?;
    Ok((
        serde_json::to_vec(input).map_err(|e| FunctionsError::InvalidLog {
            path: run.path.clone(),
            message: e.to_string(),
        })?,
        run.data.payload.export_name.clone(),
    ))
}

pub fn replay_process_spec(
    spec: &FunctionSpec,
    runner: &Path,
    replay: &ReplayRun,
    json: bool,
...
pub fn replay_watch_paths(spec: &FunctionSpec, run: &ReplayRun) -> Vec<PathBuf> {
    let mut paths = vec![spec.directory.clone(), run.path.clone()];
    paths.sort();
    paths.dedup();
    paths
}
...
impl fmt::Debug for FunctionsApiClient {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("FunctionsApiClient")
...
impl FunctionsApiClient {
    pub fn new(token: impl Into<String>, org_id: &str, app_id: &str) -> Result<Self> {
        Self::with_base(token, org_id, app_id, FUNCTIONS_API_BASE)
    }
    pub fn with_base(
        ...
        let endpoint = Url::parse(&format!("{base}/organizations/{org_id}/{app_id}/graphql"))
            .map_err(|e| FunctionsError::Api(format!("invalid Functions API endpoint: {e}")))?;
        install_crypto_provider();
        let client = reqwest::blocking::Client::builder()
            .build()
            .map_err(|e| FunctionsError::Api(e.to_string()))?;
        Ok(Self {
            client,
            endpoint,
    ...
    pub fn fetch_schema(&self, spec: &FunctionSpec) -> Result<String> {
        let version = spec.api_version.as_deref().ok_or_else(|| {
            FunctionsError::InvalidConfig(
                "function api_version is required to fetch a schema".into(),
            )
        })?;
        let (query, variables, field_path) = if let Some(target) = spec.targeting.first() {
            (
                SCHEMA_BY_TARGET,
                json!({"handle": target.target, "version": version}),
                &["target", "api", "schema", "definition"][..],
            )
        } else {
            let extension_type = spec.extension_type.as_deref().ok_or_else(|| {
                FunctionsError::InvalidConfig("function type is required to fetch a schema".into())
            })?;
            (
                SCHEMA_BY_API_TYPE,
                json!({"type": extension_type, "version": version}),
        ...
        let response = self
            ...
            .send()
            .map_err(|e| FunctionsError::Api(redact(&e.to_string(), &self.token.0)))?;
        let status = response.status();
        let body: JsonValue = response.json().map_err(|e| {
            FunctionsError::Api(redact(
                &format!("HTTP {status}: invalid JSON response: {e}"),
                &self.token.0,
            ))
        })?;
        if !status.is_success() {
            return Err(FunctionsError::Api(redact(
        ...
        for field in field_path {
            definition = definition.and_then(|value| value.get(field));
        }
        let definition = definition.and_then(JsonValue::as_str).ok_or_else(|| {
            FunctionsError::Api(
                "the Functions API returned no schema; check the Function target/type and version"
                    .into(),
            )
        })?;
    ...
    pub fn write_schema(&self, spec: &FunctionSpec, stdout: bool) -> Result<String> {
        ...
        if !stdout {
            atomic_write(&spec.schema_path, schema.as_bytes())?;
        }
        Ok(schema)
    }
}
...
pub fn atomic_write(path: &Path, contents: &[u8]) -> Result<()> {
    let parent = path.parent().ok_or_else(|| {
        FunctionsError::InvalidConfig(format!("{} has no parent directory", path.display()))
    })?;
    ...
    let result = (|| -> Result<()> {
        ...
        file.write_all(contents)
            .map_err(|source| io_error("writing temporary output", source))?;
        file.sync_all()
            .map_err(|source| io_error("syncing temporary output", source))?;
        atomic_replace(&temp, path).map_err(|source| io_error("installing output", source))
    })();
    if result.is_err() {
        let _ = fs::remove_file(temp);
    }
