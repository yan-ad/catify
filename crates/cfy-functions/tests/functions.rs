...
fn extension(directory: &Path, source: &str, family: ExtensionFamily) -> ExtensionConfig {
    ...
    ExtensionConfig {
        ...
        handle: raw.get("handle").and_then(|v| v.as_str()).map(str::to_owned),
        uid: None,
        extension_type: raw.get("type").and_then(|v| v.as_str()).map(str::to_owned),
        api_version: raw.get("api_version").and_then(|v| v.as_str()).map(str::to_owned),
        family,
        raw,
        unknown: Table::new(),
    }
...
fn parses_legacy_and_modern_function_configuration() {
    ...
    assert_eq!(legacy.targeting[0].input_query, Some(dir.join("src/run.graphql")));
...
fn selection_is_exact_or_returns_choices() {
    let a = extension(Path::new("extensions/a"), "type='cart_transform'\nname='A'", ExtensionFamily::Function);
    let b = extension(Path::new("extensions/b"), "type='cart_transform'\nname='B'", ExtensionFamily::Function);
    let graph = AppConfigGraph { root: PathBuf::from("."), apps: vec![AppNode {
        config: cfy_config::graph::AppConfig { path: "shopify.app.toml".into(), name: None, client_id: None, application_url: None, embedded: None, extension_directories: vec![], web_directories: vec![], build: Default::default(), raw: Table::new(), unknown: Table::new() },
        extensions: vec![a, b], webs: vec![],
    }], diagnostics: vec![] };
    assert_eq!(select_function(&graph, Some(Path::new("extensions/b"))).unwrap().name.as_deref(), Some("B"));
    assert!(matches!(select_function(&graph, None), Err(FunctionsError::FunctionSelectionRequired(v)) if v.len() == 2));
}
...
async fn build_runs_configured_command_and_checks_output() {
    ...
    function.build.command = Some(if cfg!(windows) { "mkdir build && echo wasm>build\\function.wasm" } else { "mkdir -p build && printf wasm > build/function.wasm" }.into());
    let output = build(&function, &Supervisor::default()).await.unwrap().unwrap();
...
async fn build_without_command_reuses_wasm_or_is_actionable() {
    ...
    assert!(matches!(build(&function, &Supervisor::default()).await, Err(FunctionsError::MissingBuild { .. })));
    fs::create_dir_all(function.wasm_path.parent().unwrap()).unwrap();
    fs::write(&function.wasm_path, b"wasm").unwrap();
    assert!(build(&function, &Supervisor::default()).await.unwrap().is_none());
}
...
fn typegen_uses_configured_command_or_lockfile_manager() {
    ...
    assert!(configured.args.iter().any(|arg| arg.contains("custom typegen")));
    ...
    assert_eq!(discovered.args, ["exec", "graphql-code-generator", "--config", "package.json"]);
}

#[test]
fn runner_urls_and_override_are_portable() {
    assert!(runner_download_url(RunnerPlatform::MacArm).ends_with("function-runner-arm-macos-v9.2.2.gz"));
    assert!(runner_download_url(RunnerPlatform::LinuxX86_64).ends_with("function-runner-x86_64-linux-v9.2.2.gz"));
    assert!(runner_download_url(RunnerPlatform::WindowsX86_64).ends_with("function-runner-x86_64-windows-v9.2.2.gz"));
    assert!(runner_platform("windows", "aarch64").is_err());
    let temp = tempdir().unwrap();
    let runner = temp.path().join("runner");
    fs::write(&runner, "").unwrap();
    assert_eq!(resolve_runner_with_override(temp.path(), Some(runner.as_os_str())).unwrap(), runner);
}
...
fn parses_lists_and_selects_replay_logs() {
    ...
    fs::write(temp.path().join(valid), r#"{"payload":{"input":{"cart":1},"export":"run"}}"#).unwrap();
    fs::write(temp.path().join("20240522_150642_827Z_extensions_discount_ghijkl.json"), r#"{"payload":{"input":null}}"#).unwrap();
    ...
    assert_eq!(serde_json::from_slice::<JsonValue>(&input).unwrap(), serde_json::json!({"cart":1}));
...
fn info_is_camel_case_and_only_reports_existing_schema() {
    ...
    assert_eq!(value["targeting"]["purchase.product-discount.run"]["export"], "run");
}

fn schema_server(token: &'static str, response_body: String) -> (String, thread::JoinHandle<String>) {
    ...
    let handle = thread::spawn(move || {
        ...
        assert!(request.to_ascii_lowercase().contains(&format!("authorization: bearer {token}").to_ascii_lowercase()));
...
fn schema_errors_do_not_expose_token() {
    ...
    let (base, handle) = schema_server(token, body);
    let temp = tempdir().unwrap();
    let error = FunctionsApiClient::with_base(token, "org", "app", &base).unwrap().fetch_schema(&spec(temp.path())).unwrap_err();
